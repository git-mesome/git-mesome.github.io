...
summary:
  [
    ...
    "role 클레임 최대 15분 stale 문제 제거, 로그아웃 세션 무효화를 AT 만료 대기 없이 다음 요청부터 즉시 반영(epoch 비교, fail-closed)",
  ]
---
## 배경
...
| 초기안                                | 채택안                                    |
| ---------------------------------- | -------------------------------------- |
| auth-service 물리 분리 + 로그인 시 gRPC 결합 | user-service가 인증·인가 통합 소유              |
| role 스냅샷 클레임 / RT는 Redis 단독        | 클레임은 sub+epoch만, RT는 RDB 원본 + Redis 캐시 |
| gateway가 Redis 직접 참조               | forward-auth로 인가 중앙화                   |
| 컨트롤러 진입점마다 세션 폐기 부착                | 상태 전이 도메인 메서드에 앵커링                     |
...
| 정지·탈퇴·승급 반영 지연 상한  | 정의 없음              | 60초 (인가 캐시 TTL)                       |
...
- **Trade-off**: user-service가 전 인증 트래픽의 동기 의존점이 되어, 재기동 시 로그인/재발급을 제외한 전 API가 잠깐 멈춥니다. 개발 환경에서의 단일 인스턴스에서는 수용한 문제, 운영 전환 시엔 다중 인스턴스가 필수 조건입니다.
...
### 2. 토큰 - 클레임은 sub+epoch만, RT는 RDB 원본 + Redis 캐시

- **문제**: role 클레임을 발급 시점 스냅샷으로 태그에 담는 방식은 최대 AT TTL(15분)만큼 낡아, 판매자 승급 같은 상태 변화가 늦게 반영됐습니다.
- **결정**: role·status처럼 신선도가 필요한 값은 토큰에 넣지 않고 매 요청 forward-auth가 원본에서 읽습니다. 클레임은 sub(uuid)와 epoch(세션 버전, RT 재발급마다 1씩 증가)만 남겼습니다 - epoch은 세션의 순번만 가리켜 role·status와 달리 stale 문제가 없습니다.
...
- **추가 도입**: 재발급마다 RT를 교체하는 RTR과 재사용 감지를 추가했습니다(아래 다이어그램).
- **Trade-off**: RTR은 백엔드·프런트 동시 배포를 강제합니다. 배포 순서(프런트가 기존 RT를 정리하기 전에 gateway만 먼저 켜면 안 됨)를 지키지 않으면 배포 즉시 전 유저 세션이 탈취로 오판되어 무효화됩니다 - 재현 조건이 아니라 결정적으로 발생하는 문제라 그레이스 기간 대신 배포 순서로 막았습니다.
...
| 대안                                  | 기각 이유                                                                                 |
| ----------------------------------- | ------------------------------------------------------------------------------------- |
| gateway가 Redis를 직접 읽어 role·블랙리스트 판정 | 트래픽 규모가 커서 앞단 차단의 이득이 클 때 유리한 패턴인데, 우리 규모에서는 그 이득보다 게이트웨이가 상태 저장소에 의존하게 되는 운영 비용이 더 큼 |
| 서비스별 인가 필터(security-starter 모듈)     | 설정·구현이 서비스마다 중복되고, 필터 누락이 곧 차단 구멍이 되는 구조                                              |


게이트웨이가 Redis 블랙리스트를 직접 들고 앞단에서 차단하는 패턴 자체는 실무에서도 흔한 선택입니다. 다만 그 패턴이 유리한 건 트래픽 규모가 커서 앞단 차단으로 도메인 서비스까지 가는 요청을 줄이는 이득이 클 때인데, 개발서버 단일 인스턴스로 돌아가는 팀 프로젝트 규모였던 상황에서는 그 이득보다 게이트웨이가 상태 저장소(Redis)에 의존하게 되면서 장애 도메인이 늘어나는 운영 비용이 더 크다고 판단했습니다. 그래서 인가 관련 상태는 전부 auth 도메인(user-service)에 두는 원칙을 일관되게 적용하는 쪽을 택했습니다.
...
- **세부값 근거**: 정책표(경로→필요 role)는 `@ConfigurationProperties`로 외부화했지만, `/admin/**` → ADMIN 캐치올만은 설정이 아니라 코드에 하드코딩된 기본값으로 고정했습니다 - 설정 파일 오류로 가장 위험한 보호선이 조용히 사라지는 걸 막기 위해서입니다. 정책표가 비거나 파싱에 실패하면 기동 자체를 fail-fast로 중단시킵니다.
- **장애 대응**: authorize 응답은 60초 TTL로 캐시하고(상태 변경 시 즉시 무효화), Redis 장애 시엔 DB 직접 조회로 폴백합니다 - 인가는 fail-open이 불가능한 값이라 느려지는 것까지만 허용했습니다.
...
- **결정**: 이 부수효과를 컨트롤러가 아니라 user-service의 상태 전이 도메인 메서드(`user.block()` 등)에 붙여, 호출자가 누구든 상태가 바뀌는 지점을 하나로 고정했습니다. admin-service는 스키마 전체에 읽기·쓰기 권한이 있지만 status·role 변경만은 반드시 user-service API를 경유하게 해, 직접 UPDATE로 우회되는 경로를 막았습니다(우회해도 staleness는 캐시 TTL 60초 제한).
